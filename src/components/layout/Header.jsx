...
const Header = () => {
  ...
  return (
    ...
    <header className={`${isScrolled ? 'bg-[#0a0e17]/95 backdrop-blur-md shadow-2xl border-b border-white/5' : 'bg-gradient-to-b from-black/90 via-black/40 to-transparent'} fixed top-0 w-full z-[100] transition-all duration-500 h-16 md:h-20`}>
      <div className="w-full h-full px-4 md:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 lg:gap-10">
            {/* Mobile Menu Button */}
            <button className="lg:hidden text-white text-2xl p-2 hover:bg-white/10 rounded-lg transition" onClick={() => setMobileMenuOpen(true)}>
                <FaBars />
            </button>
    ...
    {/* --- MOBILE SIDEBAR (ĐƯỢC THIẾT KẾ LẠI) --- */}
    ...
    <div className={`fixed top-0 left-0 w-[85%] max-w-[320px] h-full bg-[#111] z-[200] transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl border-r border-white/10`}>
        ...
        <div className="p-6 border-b border-white/10 bg-gradient-to-b from-black/40 to-transparent">
            <div className="flex justify-between items-start mb-4">
                {user ? (
                    <div className="flex items-center gap-3" onClick={() => navigate('/ho-so')}>
                        <div className="w-12 h-12 rounded-full border-2 border-phim-accent overflow-hidden">
                            <img src={user.avatar} alt="User" className="w-full h-full object-cover" />
                        </div>
                        <div>
            ...
            {user && (
                <div className="grid grid-cols-3 gap-2 mt-4">
                    <button onClick={() => { navigate('/tu-phim'); setMobileMenuOpen(false); }} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 hover:bg-white/10 transition">
                        <FaHeart className="text-red-500" /> <span className="text-[10px] text-gray-300">Yêu thích</span>
                    </button>
                    <button onClick={() => { navigate('/lich-su'); setMobileMenuOpen(false); }} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 hover:bg-white/10 transition">
                        <FaHistory className="text-blue-400" /> <span className="text-[10px] text-gray-300">Lịch sử</span>
                    </button>
                    <button onClick={() => { navigate('/ho-so'); setMobileMenuOpen(false); }} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/5 hover:bg-white/10 transition">
                        <FaCog className="text-gray-400" /> <span className="text-[10px] text-gray-300">Cài đặt</span>
                    </button>
                </div>
            )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
            ...
            {/* Dropdown: Danh Sách */}
            <div>
                <button onClick={() => toggleMobileSubmenu('danh-sach')} className="w-full flex items-center justify-between py-3 px-4 rounded-lg text-gray-300 hover:text-white hover:bg-white/5 transition font-medium">
            ...
            {/* Dropdown: Thể Loại */}
            <div>
                <button onClick={() => toggleMobileSubmenu('the-loai')} className="w-full flex items-center justify-between py-3 px-4 rounded-lg text-gray-300 hover:text-white hover:bg-white/5 transition font-medium">
                ...
                <div className={`pl-11 pr-2 overflow-hidden transition-all duration-300 ${activeMobileSubmenu === 'the-loai' ? 'max-h-[500px] opacity-100 pb-2' : 'max-h-0 opacity-0'}`}>
                    <div className="grid grid-cols-2 gap-2 border-l border-white/10 pl-3 pt-2">
                        {menuData.theLoai.slice(0, 12).map((item) => (
                            <a key={item._id} href={`/the-loai/${item.slug}`} className="text-sm text-gray-500 py-1 hover:text-white truncate">{item.name}</a>
                        ))}
                        <a href="/the-loai" className="text-sm text-phim-accent font-bold py-1 col-span-2 flex items-center gap-1">Xem tất cả <FaArrowRight size={10}/></a>
                    </div>
                </div>
            </div>
        </div>

        {/* 3. MOBILE FOOTER (AUTH ACTIONS) */}
