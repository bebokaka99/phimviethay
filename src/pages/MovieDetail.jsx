...
import { FaPlay, FaClock, FaGlobe, FaStar, FaShareAlt, FaHeart, FaChevronDown, FaChevronUp, FaChevronLeft, FaChevronRight, FaCamera, FaUserEdit, FaSignOutAlt, FaCheckCircle, FaCloudUploadAlt, FaTrashAlt } from 'react-icons/fa';
...
const MovieDetail = () => {
  ...
  const getRandomGradient = (name) => {
      const gradients = ['from-red-600 to-rose-900', 'from-blue-600 to-indigo-900', 'from-emerald-600 to-teal-900', 'from-violet-600 to-purple-900', 'from-amber-500 to-orange-900'];
      let hash = 0;
  ...
  // --- CHUẨN BỊ DATA SEO ---
  const movieTitle = `${movie.name} (${movie.year}) - Xem Phim HD`;
  const movieDesc = stripHtml(movie.content).substring(0, 160);
              ...
              <div className="flex flex-col md:flex-row gap-6 md:gap-14 items-center md:items-start">
                  ...
                  <div className="flex-1 space-y-5 md:space-y-7 z-20 text-center md:text-left w-full">
                      ...
                      <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 md:gap-6 text-xs md:text-sm font-bold text-gray-300">
                          <div className="flex items-center gap-1 text-yellow-400 text-base"><FaStar /> <span>{movie.vote_average || 8.5}</span></div>
                      ...
                      <div className="text-sm md:text-base text-gray-300 leading-loose max-w-3xl mx-auto md:mx-0">
                          <p className={isExpanded ? '' : 'line-clamp-3 md:line-clamp-4'}>{stripHtml(movie.content)}</p>
              ...
              <div ref={castRef} className="flex overflow-x-auto gap-4 pb-4 scrollbar-hide snap-x scroll-smooth">
                  {movie.director?.map((dir, idx) => (
                      <div key={`dir-${idx}`} className="flex flex-col items-center min-w-[80px] snap-start">
                          <div className="w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center text-sm font-bold shadow-md mb-2 border border-gray-600 bg-gray-800 text-gray-400">{getInitials(dir)}</div>
                  ...
                  {movie.actor?.map((act, idx) => (
                      <div key={`act-${idx}`} className="flex flex-col items-center min-w-[80px] snap-start">
                          <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center text-sm font-bold shadow-md mb-2 border border-transparent bg-gradient-to-br ${getRandomGradient(act)}`}>{getInitials(act)}</div>
