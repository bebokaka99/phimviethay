...
const VideoPlayer = ({ movieSlug, option, style, episodes, servers, currentEp, onEpChange, onServerChange, currentServerIndex, onNextEp, hasNextEp, ...rest }) => {
    ...
    useEffect(() => {
        ...
        // THÊM pointer-events-auto VÀO STYLE ĐỂ PANEL LUÔN BẤM ĐƯỢC
            ...
            if (hasNextEp && art.duration - art.currentTime <= 90 && art.currentTime > 0) {
                if (!autoNextTimer.current) {
                    ...
                    autoNextTimer.current = setInterval(() => {
                        const currentRemain = Math.ceil(art.duration - art.currentTime);
                        setNextCount(currentRemain);
                        if (currentRemain <= 0) {
                            clearInterval(autoNextTimer.current);
                            autoNextTimer.current = null;
                            if (onNextEpRef.current) onNextEpRef.current();
                        }
                    }, 1000);
                }
            } else {
    ...
    useEffect(() => {
        ...
        ['server-panel', 'episode-panel'].forEach(name => {
            ...
            Array.from(container.querySelectorAll('[data-index], [data-slug]')).forEach(item => {
                item.onclick = (e) => {
                    e.stopPropagation(); // CHẶN LAN TRUYỀN ĐỂ KHÔNG BỊ PAUSE
                    const p = art.layers[name];
    ...
    return (
        <div className="relative group overflow-hidden w-full h-full" style={style}>
            ...
            {showAutoNext && (
                <div onClick={handleManualNext} className="absolute bottom-16 right-4 z-[100] w-80 bg-white/10 backdrop-blur-xl border border-red-600/40 rounded-2xl overflow-hidden shadow-2xl animate-in slide-in-from-right duration-500 cursor-pointer pointer-events-auto">
                    <div className="p-4 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="text-[10px] text-white/50 uppercase font-black tracking-widest leading-none mb-1">Tập sau trong {nextCount}s</div>
                            <div className="text-sm text-white font-bold truncate leading-tight">Bấm để chuyển ngay...</div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); setShowAutoNext(false); clearInterval(autoNextTimer.current); autoNextTimer.current = null; }} className="p-1 text-white/50 hover:text-white"><FaTimes size={14} /></button>
                    </div>
