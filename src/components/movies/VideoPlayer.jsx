...
import { FaMicrophone, FaList, FaStepForward, FaForward, FaTimes, FaMagic } from 'react-icons/fa';
...
const VideoPlayer = ({ movieSlug, option, style, episodes, servers, currentEp, onEpChange, onServerChange, currentServerIndex, onNextEp, hasNextEp, ...rest }) => {
    ...
    useEffect(() => {
        ...
        isAutoNextDismissed.current = false;
        isIntroDismissed.current = false; 

        if (movieSlug && currentEp?.slug) {
            getEpisodeIntelligence(movieSlug, currentEp.slug).then(data => {
                if (data) introDataRef.current = data;
            });
        }
    }, [movieSlug, currentEp]);
        ...
        const handleKeyDown = async (e) => {
            ...
            if (e.altKey && e.key.toLowerCase() === 'o') {
                ...
                if (confirm(`💾 GOD MODE: Lưu Intro?\n${start.toFixed(1)}s -> ${end.toFixed(1)}s`)) {
                    try {
                        await forceIntroData({
                            movie_slug: movieSlug, episode_slug: currentEp?.slug,
                            intro_start: Math.round(start), intro_end: Math.round(end)
                        });
            ...
            if (e.altKey && e.key.toLowerCase() === 'c') {
                ...
                if (confirm(`💾 GOD MODE: Lưu Auto Next?\nTại: ${art.currentTime.toFixed(1)}s`)) {
                    try {
                        await forceIntroData({
                            movie_slug: movieSlug, episode_slug: currentEp?.slug,
                            credits_start: Math.round(art.currentTime)
                        });
    ...
    const checkTimeUpdate = (art) => {
        ...
        if (intro && intro.intro_end && !isIntroDismissed.current) { 
            if (currentTime >= intro.intro_start && currentTime < intro.intro_end) {
                shouldShowSkip = true;
            }
        }
        ...
        // 2. AUTO NEXT
        if (isAutoNextDismissed.current || !hasNextEp) {
            setShowAutoNext(false);
            return;
        }

        let isEnding = false;
        if (intro && intro.credits_start) {
            if (currentTime >= intro.credits_start) {
                isEnding = true;
            }
        }
        ...
        if (isEnding) {
            ...
            if (remaining <= 1 && onNextEpRef.current) {
                onNextEpRef.current();
            }
        } else {
            setShowAutoNext(false);
        ...
        ['server-panel', 'episode-panel'].forEach(name => {
            ...
            Array.from(container.querySelectorAll('[data-index], [data-slug]')).forEach(item => {
                item.onclick = (e) => {
                    e.stopPropagation(); 
                    const p = art.layers[name];
                    p.querySelector('.art-panel-drawer').style.right = '-100%';
                    setTimeout(() => p.style.display = 'none', 300);
                    if (name === 'server-panel') { 
        ...
        <div className="relative group overflow-hidden w-full h-full" style={style}>
            ...
            {showSmartSkip && (
                <div 
                    ...
                    className="absolute bottom-24 right-4 z-[160] w-64 cursor-pointer group/skip animate-in pointer-events-auto"
                >
                    <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 group-hover/skip:bg-black/80 group-hover/skip:border-red-500/50">
                        <div className="p-3 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white group-hover/skip:bg-red-600 group-hover/skip:text-white transition-colors duration-300">
                                <FaForward size={14} className="ml-0.5" />
                            </div>
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                <div className="text-[10px] text-white/50 font-black tracking-widest uppercase mb-0.5 group-hover/skip:text-red-400 transition-colors">
                                    Giới thiệu
                                </div>
            ...
            {showAutoNext && (
                <div onClick={handleManualNext} className="absolute bottom-24 right-4 z-[160] w-64 cursor-pointer group/next animate-in pointer-events-auto">
                    <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 group-hover/next:bg-black/80 group-hover/next:border-red-500/50">
                        <div className="p-3 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white group-hover/next:bg-red-600 group-hover/next:text-white transition-colors duration-300">
                                <FaStepForward size={14} className="ml-0.5" />
                            </div>
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                <div className="text-[10px] text-white/50 font-black tracking-widest uppercase mb-0.5 group-hover/next:text-red-400 transition-colors">
                                    Tập sau trong {nextCount}s
                                </div>
                                <div className="text-sm text-white font-bold leading-none truncate">
                                    Tập tiếp theo
                                </div>
                            </div>
                            <button 
                                onClick={(e) => { 
                                    e.stopPropagation(); 
                                    setShowAutoNext(false); 
                                    isAutoNextDismissed.current = true;
                                }} 
                                className="p-2 text-white/30 hover:text-white transition-colors"
                            >
                                <FaTimes size={12} />
            ...
            {showResume && (
                <div className="absolute inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[180] pointer-events-auto">
                    <div className="p-8 rounded-3xl bg-white/10 border border-red-600/30 backdrop-blur-2xl shadow-2xl flex flex-col items-center max-w-[320px]">
                        ...
                        <h3 className="text-white text-lg font-bold mb-1 uppercase tracking-wider">Xem tiếp?</h3>
                        <p className="text-gray-300 text-sm mb-6 text-center">Đang dừng tại <span className="text-red-500 font-mono font-bold mx-1">{new Date(resumeTime * 1000).toISOString().substr(11, 8)}</span></p>
                        <div className="flex gap-3 w-full">
                            <button onClick={() => { setShowResume(false); playerRef.current.template.$state.style.display = ''; playerRef.current.play(); localStorage.removeItem(getSafeStorageKey()); }} className="flex-1 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-bold transition-all">XEM LẠI</button>
                            <button onClick={() => { setShowResume(false); playerRef.current.template.$state.style.display = ''; playerRef.current.currentTime = resumeTime; playerRef.current.play(); }} className="flex-1 py-2 rounded-xl bg-red-600 text-white text-xs font-bold transition-all shadow-lg shadow-red-600/30">TIẾP TỤC</button>
                        </div>
                    </div>
                </div>
            )}
